...
{
    internal class FusionConnection : IFusionConnection
    {
        public FusionConnection(IFusionConnectConfig config, NetworkRunner runnerPrefab)
        {
            _config = config;
        ...
        private IFusionConnectConfig _config;
        private bool _connectingSafeCheck;
        private CancellationTokenSource _cancellationTokenSource;
        private CancellationToken _cancellationToken;

        public async Task<ConnectResult> ConnectAsync(IFusionConnectArgs connectArgs)
        {
            // Safety
        ...
        public Task<List<FusionOnlineRegion>> RequestAvailableOnlineRegionsAsync(IFusionConnectArgs connectArgs)
        {
            // Force best region
            return Task.FromResult(new List<FusionOnlineRegion>() { new FusionOnlineRegion() { Code = string.Empty, Ping = 0 } });
        }
        ...
        private FusionAppSettings CopyAppSettings(IFusionConnectArgs connectArgs)
        {
